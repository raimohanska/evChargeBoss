...
- [x] MQTT status messages are now missing after clean-up. Log below. You see that the relay was turned OFF, but status not updated. Update tests so that this would fail and then fix properly. See if a refactoring is possible to ensure that status updates and relay control are always in sync.
